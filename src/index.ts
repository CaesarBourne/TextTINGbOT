...
// };
export const parseTextBotCommand = (rawInput: string, groups: Group[]): ParseTextBotCommandOutput | null => {
    const commandRegex = /^txt\s+/i;
    const match = rawInput.match(commandRegex);
    if (!match) return null;
    
    const inputWithoutCommand = rawInput.substring(match[0].length).trim();
    if (inputWithoutCommand === '') return null;
    ...
    for (const group of groupsWithNorm) {
        const nonSpaceCount = group.normalized.length;
        let count = 0;
        let index = 0;

        for (index = 0; index < inputWithoutCommand.length && count < nonSpaceCount; index++) {
            const c = inputWithoutCommand[index];
            if (/\s/.test(c)) continue;
            count++;
        }

        if (count !== nonSpaceCount) continue;

        if (index < inputWithoutCommand.length && !/\s/.test(inputWithoutCommand[index])) {
            continue;
        }

        const messageToSend = inputWithoutCommand.substring(index).trim();
        return {
